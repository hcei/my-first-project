...
- **GUI（翻页条第二行）**：`[✓]蓝牙翻页　档位(0~50)：[ ]　时长(ms)：[ ]　[连接蓝牙]　状态文本`。档位/时长沿用两段式整数框（EN_CHANGE 合法即套用不回写、KILLFOCUS 夹取回写）；`WritePlotGeo` 新增 `strip2` 并把下方四角标定区整行下移；新增复选框**必须紧邻 `IDC_CHECK_*` 块**（`MakeCheck` 按 `id-IDC_CHECK_DRY` 索引）；按钮要加进 `DrawBtn` 的颜色组。`WM_DESTROY` 调 `gs::page_turn_shutdown()`（断开 GATT + 停线程；不做的话下次启动可能撞 SharingViolation）。
- **config 新键**：`page_turn_ble`(默认 false)/`page_turn_gear`(0~50，默认 30)/`page_turn_run_ms`(100~600000，默认 3000)/`ble_addr`(默认 `21F6473AD889`)。审计新事件 `page_turn_ble`（cmd/gear/run_ms/result/elapsed_ms/detail，建链失败也记一条）。
