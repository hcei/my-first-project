...
| 待修复 2026-09-18 | **同类隐患未消除：点“连接设备”仍会在 UI 线程上打开串口**。`OnCommand` 的 `IDC_BTN_CONNECT` 直接调用 `gs::connect()`，而 `gs::connect` 在持有 `g_mu` 的情况下 `g_port.open()`；若用户选中上例中任何一个蓝牙虚拟串口，CreateFile 会在内核阻塞 → 界面卡死（且 `g_mu` 被长期持有，连 500ms 的 `snapshot()` 计时器也一起阻塞） | 设备连接页“连接设备”按钮 | 选 COM3~COM9 任一端口 → 点“连接设备” → 界面无响应（本机这 6 个端口全是蓝牙虚拟串口，必然触发）。修复方向：连接改到工作线程 + 打开串口期间不持有 `g_mu`，完成后经计时器回报结果；涉及共享服务层加锁语义，需先确认再改 |
